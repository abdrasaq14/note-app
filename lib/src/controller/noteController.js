...
const createNewNoteFunction = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.UserId;
        const generateNoteID = (0, uuidFunction_1.generateUUID)().slice(9, 13);
    ...
    VALUES (?,?,?,?,?,?)`;
        if (userId === undefined) {
            ...
            const createNewNote = yield new Promise((resolve, reject) => {
                db.run(sql, [generateNoteID, Title, Description, DueDate, Status, userId], function (err) {
                    if (err) {
                        reject(err);
                    }
                    else {
    ...
    WHERE
      NoteId = ?`;
        const insertUpdateNote = yield new Promise((resolve, reject) => {
            db.run(sql, [Title, Description, DueDate, Status, noteId], function (err) {
                if (err) {
                    reject(err);
                }
                else {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const deletedNotes = yield new Promise((resolve, reject) => {
                db.run(sql, [noteCode], function (err) {
                    if (err) {
                        reject(err); // Reject with the error
                    }
                    else {
